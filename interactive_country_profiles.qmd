---
title: "UPR Health Explorer"
# subtitle: "An interactive exploration of health-related recommendations from the first four cycles of the Universal Periodic Review"
# author: "Anshu Uppal"
...
output$regional_map <- renderPlot({
  p1 <- state_geo_reactive() |> 
    ggplot(aes(geometry = polygon, fill = region_dashboard))+
    geom_sf()+
    scale_color_manual(values = c("green4", "grey80"))+
    scale_linewidth_manual(values=c(0.8, 0.3))+
    scale_fill_manual(values = c("green4", "grey90"))+
...
Grouping by Fragile/Conflict-affected Situations (**FCS status**) was made according to the following grouping obtained from the World Bank:
...
The Right to Health
:::
The Right to Health is central to the fulfillment of broader human rights obligations, serving as a powerful tool to advance well-being, equity, and dignity across all sectors of society.  

The Right to Health comprises the State's obligations to:  
...
-  Each cycle repeats every 4-5 years - see below illustration for an overview of each cycle  
![](www/UPR_review_banner2.png){width=30%}  

The Reviews are guided by three main pre-session reports (see below):  
-  Reviewing States issue recommendations  
-  The State Under Review can either “Support” (accept) or “Note” each recommendation  
![](www/UPR_pre_review.png){width=60%}
...
CeHDI
:::
[CeHDI](https://www.cehdi.org/) has a mission of amplifying and facilitating the inclusion of  the priorities and voices of the Global South within the global health architecture and building robust partnerships for global health equity and the right to health.

::: {.highlight-block}
Preliminary results?
:::
![](www/full_plot.png){width=50%}
...
output$mmr_map <- renderPlot({
  ...
  p1<-MMR |> 
    ...
    right_join(state_geo_reactive(), join_by(COUNTRY == iso3)) |> 
    ...
    labs(title = paste0("Maternal mortality ratio (MMR) estimate in 2023\n", input$selected_SUR, ": ", mmr_estimate_2023, " per 100,000 live births"),
         fill = NULL,
         color = NULL, lwd = NULL)+
...
### Row {height=55%}
```{r}
#| title: MMR trends with comparison with neighboring States
...
```{r}
#| title: Causes over time
plotOutput("mmr_causes_longitudinal")
```
...
output$abortion_rate <- renderPlot({
  p1<-abortion_rate |> 
    ...
    scale_linewidth_manual(values=c(0.8, 0.3))+
    scale_color_manual(values = c("blue3", "grey90"))+
    scale_fill_stepsn(n.breaks = 8,  na.value = "grey80",
